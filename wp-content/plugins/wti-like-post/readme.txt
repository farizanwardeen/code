...
= PRO Features =
1. myCRED points integration for liking and unliking posts.
2. Functionality to show user avatars who liked the post.
3. Ability to change your vote even if multiple voting is disabled.
4. Action hook available for successful voting so that you can attach any functionality like sending thank you mail to user. There are 2 other hooks available for further customizing the message you see on page load and ajax call.
5. Functionality for viewing like, unlike, total counts for individual post on admin Most Liked Posts screen. You can also view users who voted for each of them. This works only when logged in users are allowed to vote.
6. Buddypress activity integration to show activity updates when a user likes and dislikes any standard or custom post. There will not be option to like/dislike any BuddyPress activity.
7. Functionality for highlighting the like/unlike icon on successful voting and page load.
8. Compatible with multisite set up.
9. Can be used with custom post types. You can show most liked posts based on custom post types using shortcode and widget.
10. Functionality for disabling the plugin css file so that you can use custom css from your theme.
11. Functionality for redirecting user to a specific page on successful voting.
12. Functionality for showing the like/unlike buttons for post excerpts.
13. Functionality for allowing/disallowing author to vote against own post with custom message.
14. Option to save plugin settings and table after plugin un-installation in case you need to reuse the data in future.
15. Functionality to disable auto-loading of the like/unlike buttons so that you can use the template tag <?php GetWtiLikePost()?> inside your theme or [wtilp_buttons] inside post/page content using the editor. Please note that [wtilp_buttons] is NOT a shortcode.
16. Functionality to store voting counts (like, unlike, total) as post meta which can be used to show posts sorted by voting counts. 
17. Functionality to have default message to encourage users to like posts. 
18. Functionality to show users who liked a post below the like/unlike buttons. 
19. In total 6 styles of buttons for like/unlike functionality.
20. Functionality to show like count for each post in admin post list/edit section.
21. Has 2 template files which can be used to show most liked posts throughout the site and the most liked posts by an author on author page.
22. Functionality to use texts instead of like/unlike images in case you want to have some encouraging texts which can not be conveyed using images.
23. Functionality for adding default like/unlike entries i.e. 0 for posts and pages when they are created.
24. Functionality to show most liked/unliked posts from selected categories on Most Liked/Unliked Posts widget.
25. Functionality to show posts liked/unliked by all users or the logged in user on the widget and also shortcode for showing the same on a page.
26. Wide range of time including hours to have more control on the posts shown on the widget.
27. Most Liked/Unliked Category Posts widget to show posts liked/unliked on the specific category page.
28. Functionality to show post excerpt, thumbnail on all the available widgets.
